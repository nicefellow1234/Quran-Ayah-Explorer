...
export function ReaderModeToggle() {
  ...
  return (
    <div className="reader-mode-toggle" aria-label="Reading mode">
      <button type="button" className={!readingMode ? "is-active" : ""} onClick={() => setMode("verse")} aria-pressed={!readingMode}>
        <List size={14} aria-hidden="true" />
        <span>Verse by Verse</span>
      </button>
      <button type="button" className={readingMode ? "is-active" : ""} onClick={() => setMode("reading")} aria-pressed={readingMode}>
