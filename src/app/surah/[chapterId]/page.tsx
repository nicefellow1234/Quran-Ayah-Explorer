...
type PageProps = { params: Promise<{ chapterId: string }>; searchParams: Promise<{ translation?: string | string[]; mode?: string; view?: string; translationView?: string }> };
  ...
  return (
    <>
      ...
      <main className={`shell reader-page${readingMode ? " reading-mode" : ""}`}>
        ...
        {readingMode ? <ReadingModeHeader chapter={chapter} view={readingView} defaultReciterId={getDefaultReciterId(resources)} translations={resources.translations} selectedTranslationIds={translationIds} selectedTranslationId={readingTranslationId} /> : <header className="reader-header">
        ...
        {chapter.bismillahPre && (!readingMode || readingView === "arabic") ? <div className={`bismillah-divider${readingMode ? " reading-bismillah" : ""}`} aria-label="Opening of the Surah">
        ...
        <ReaderClient key={`surah-${id}-${contentTranslationIds.join(",")}-${readingMode ? "reading" : "verse"}`} chapterId={id} totalVerses={chapter.versesCount} translationIds={contentTranslationIds} queueVerseKeys={Array.from({ length: chapter.versesCount }, (_, index) => `${id}:${index + 1}`)} verses={verses} tafsirs={resources.tafsirs} reciters={resources.reciters} defaultTafsirId={getDefaultTafsirId(resources)} defaultReciterId={getDefaultReciterId(resources)} readingMode={readingMode} readingView={readingMode ? readingView : "both"} readingTranslationId={readingMode ? readingTranslationId : undefined} />
